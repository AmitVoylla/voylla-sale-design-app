            ...
            with col2:
                if item['visualizations']:
                    ...
                    for viz in item['visualizations']:
                        st.plotly_chart(viz, width=True)
                
                if not item['data'].empty:
                    st.markdown("#### 📋 Data")
                    st.dataframe(item['data'].head(), width=True)
...
if inp:
    ...
    if visualizations:
        ...
        if len(visualizations) == 1:
            st.plotly_chart(visualizations[0], width=True)
        elif len(visualizations) == 2:
            col1, col2 = st.columns(2)
            col1.plotly_chart(visualizations[0], width=True)
            col2.plotly_chart(visualizations[1], width=True)
        else:
            for i in range(0, len(visualizations), 2):
                cols = st.columns(2)
                for j, viz in enumerate(visualizations[i:i+2]):
                    cols[j].plotly_chart(viz, width=True)
