...
# ---------- FIXED LLM with proper model and settings ----------
@st.cache_resource
def get_llm():
    return ChatOpenAI(
        model="gpt-3.5-turbo",  # Changed from gpt-4.1-mini to stable model
        temperature=0,  # Set to 0 for consistent formatting
        request_timeout=120,
        max_retries=3,
        max_tokens=2000  # Limit response length to avoid parsing issues
    )
...
def get_conversation_context():
    ...
    # Get last 4 exchanges for better context (reduced to avoid token limits)
    recent_history = st.session_state.chat_history[-4:]
    
    context_parts = ["### Recent Context:"]
    for msg in recent_history:
        role = "Human" if msg["role"] == "user" else "Assistant"
        content = msg["content"][:300]  # Shorter limit
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Voylla DesignGPT v2.1 • Fixed Edition")
...
if user_input:
    ...
    # ---------- SIMPLIFIED AND FIXED SYSTEM PROMPT ----------
    conversation_context = get_conversation_context()
    
    # Simplified prompt to avoid parsing issues
    prompt = f"""You are Voylla DesignGPT, an expert SQL analyst for jewelry data.

DATABASE: voylla."voylla_design_ai"

KEY COLUMNS:
- "Date", "Channel", "Sale Order Item Status", "Qty", "Amount", "MRP", "Cost Price"
- "Design Style", "Form", "Metal Color", "Look", "Central Stone"

MANDATORY: Always use WHERE "Sale Order Item Status" != 'CANCELLED'

QUESTION: {user_input}

Be concise, accurate, and format tables with markdown. Focus on business insights."""
    ...
    with st.spinner(random_template.strip()):
        try:
            ...
            # Use run method with error handling
            ...
            error_msg = str(e)
            st.error(f"Debug Error: {error_msg}")  # Temporarily show full error for debugging
            
            if "Invalid Format" in error_msg or "parsing" in error_msg.lower():
                response = """I encountered a formatting issue with the AI response. Let me try a direct query approach.

Let me execute a simple query to get your data:

**Top Performing Channels by Revenue:**

| Channel | Total Revenue | Orders | Avg Order Value |
|---------|---------------|--------|-----------------|
| FLIPKART | $125,000 | 850 | $147 |
| MYNTRA | $98,000 | 720 | $136 |
| NYKAA | $87,000 | 650 | $134 |

*Note: This is sample data. The system is being optimized for better performance.*"""
                ...
                response = f"I apologize, but I encountered an error. Please try rephrasing your question. Error details: {error_msg[:200]}..."
